...
- Owner-scoped cancellation guides with dual-layer URL, text, phone, and verification-date validation.
- Same-origin bounded JSON APIs for password-reauthenticated export and current-account deletion.
- Rate-limited one-time account-action permits bound to a newly issued verified Auth session.
- Versioned owner-only JSON export and full Auth-user cascade deletion with anonymous content-free receipts.
...
- Account-action attempts use opportunistic 24-hour cleanup; scheduled hosted cleanup remains unimplemented.

### What is planned

- Production backup expiry, scheduled retention, support access, and incident operations.
...
| Price history                                     | High           | Owner-scoped immutable history   | Integrity, owner isolation, confirmation workflow                                |
| Reminder records                                  | Medium         | Owner-scoped in-app records      | Owner isolation, safe delivery, no sensitive payload in notifications            |
| Budget and savings values                         | High           | Owner-scoped PostgreSQL          | Owner isolation, no logs, accurate integer calculations                          |
| Cancellation URLs, instructions, phone, and notes | Medium to High | Owner-scoped private guides      | HTTPS allowlist, provenance, owner authorization, anti-phishing copy             |
| Account JSON exports                              | High           | Generated after reauthentication | Owner isolation, no-store delivery, no tokens or source bytes                    |
| Anonymous deletion receipts                       | Low            | Private content-free records     | No retained identity or financial details, no client access                      |
...
- Same-origin `POST /api/account/export` and `DELETE /api/account` JSON bodies.
- Cancellation-guide Server Actions and dynamic owner subscription IDs.
...
| TM-02 | Spoofing; stolen-session attacker                        | Session and account; every protected operation              | A stolen session remains valid through logout, password reset, or excessive inactivity.                                      | HttpOnly `SameSite=Lax` cookies, rotation, bounded sessions, current-password reauthentication, new-session permits, and Auth-user deletion.            | Hosted provider revocation and recent-auth verification.                            | Low locally | High     | Verify hosted global revocation and Auth configuration before deployment.                                                                                        |
...
| TM-16 | Spoofing/injection; compromised guide author             | Cancellation URL; database-to-browser                       | A trusted-looking guide directs the user to a malicious or non-HTTPS destination.                                            | Owner-only guides, dual-layer HTTPS and control validation, visible hostname, `noopener noreferrer`, verification date, and provider-confirmation copy. | Provenance and moderation if shared guidance is ever added.                         | Low         | High     | Keep shared guidance out of scope until a separate moderation and provenance model is approved.                                                                  |
...
| TM-21 | Disclosure; stolen session or cache                      | Account export; browser-to-server and response boundary     | An attacker exports private records through a stolen session or receives another user's cached download.                     | Password reauthentication, new-session-bound permit, recent JWT, owner-derived export, fixed JSON filename, and private no-store response.              | Hosted proxy and large-export memory verification.                                  | Low locally | Critical | Preserve step-up authentication and test cache, size, and isolation against production-equivalent infrastructure.                                                |
| TM-22 | Tampering; stolen session or malicious user              | Account deletion; browser-to-server and Auth/database       | An attacker deletes another account, replays a permit, or leaves owner records or sessions behind.                           | Current-user-only RPC, exact phrase, password reauthentication, one-time permit, Auth cascade, cookie expiry, full-table tests, and anonymous receipt.  | Hosted global session-revocation and backup-expiry verification.                    | Low locally | Critical | Test deployed session invalidation and document backup deletion before production approval.                                                                      |
...
| `cancellation_guides`        | Allow owner                                     | Controlled RPC         | Controlled RPC             | Controlled RPC       | Deny all  | Deny all   | Private owner-maintained guides only; optimistic writes and dual-layer URL/content validation                          |
...
| Cleanup           | Keep raw bytes in bounded request memory only, clear application buffers on every success and failure path, and never persist source files                                       |
...
| ------------------- | ------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------- |
| Input encoding      | React escaping plus strict URL, CSV, form, and JSON validation in application and database layers            | Fuzzing and deployed edge validation                                 |
| Authentication      | Supabase SSR, verified claims, HttpOnly cookies, bounded sessions, password step-up, and new-session permits | Hosted environment and provider configuration review                 |
| Authorization       | Server identity checks, minimal DAL, explicit grants, forced RLS, controlled RPCs, and 191 pgTAP assertions  | Deployment database-role and support-access review                   |
| Data minimization   | Route-specific SQL selection, exact browser DTOs, versioned export schema, and no raw source persistence     | Continue payload and export reviews                                  |
...
| Browser storage     | Theme name only; auth tokens remain in HttpOnly cookies                                                      | Keep all financial values and exports out of web storage             |
| Logging             | Content-free audit events with no direct client access                                                       | Deployment retention, alerting, integrity, and access controls       |
| File handling       | Bounded authenticated CSV import with memory-only source bytes                                               | Production resource and crash-dump verification                      |
| Dependency controls | Lockfile, integrity, audit, and production signature verification                                            | Continuous monitoring, isolated CI, reviewed lifecycle changes       |
| Browser headers     | Tested CSP and supporting response headers                                                                   | Hosted CSP, HSTS, and proxy review                                   |
| Error handling      | Generic action and API errors, protected boundary, and owner-blind 404                                       | Stable operational monitoring                                        |
| Privacy             | Accurate local notice, private JSON export, Auth cascade deletion, and anonymous receipt                     | Backup expiry, scheduled cleanup, legal, and support-access controls |
